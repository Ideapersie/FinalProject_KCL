...
\begin{table}[t]
  ...
  \begin{tabular}{llccccc}
    ...
    \midrule
    P1 always-retrieve   & MCQ  & Acc. & 17\%    & \textbf{46\%}  & 100\% & 27\,s \\
    P4 hybrid            & MCQ  & Acc. & ---     & 46\%           & 100\% & 25\,s \\
    P3 closed-book$^{*}$ & MCQ  & Acc. & 62\%    & 62\%           & 0\%   & 57\,s \\
    P5 gated (calib.)    & MCQ  & Acc. & 45.5\%  & \textbf{54\%}  & 50\%  & 153\,s \\
    \midrule
    P1 always-retrieve   & open & F1   & ---     & \textbf{0.220} & 100\% & 23\,s \\
    P3 closed-book$^{*}$ & open & F1   & 0.190   & 0.190          & 0\%   & 27\,s \\
    P5 gated (calib.)    & open & F1   & 0.191   & \textbf{0.211} & 79\%  & 380\,s \\
    \bottomrule
  \end{tabular}
  \caption{All policies on the real MedCorp corpus (425{,}847 chunks) versus the
  200-chunk pilot. Quantised Llama-3.2-3B, medium tier. $^{*}$P3 is closed-book,
  so its numbers are corpus-independent references.}
...
accuracy rose from 17\% to 46\%, and open-ended F1 rose across the board
...
Among the retrieving policies, the gated policy P5 is the best: 54\% versus 46\%
for both P1 (always) and P4 (hybrid), while retrieving on only \emph{half} the
queries (Figure~\ref{fig:medcorp-policies}). This is the project's central claim
made concrete. By skipping retrieval on the questions the model already answers
confidently, P5 avoids the distraction that unnecessary context injects, and it
does so at half the retrieval cost. The gate is not merely reducing compute for
free --- on multiple choice it is \emph{more accurate} than retrieving every time.
...
\begin{figure}[t]
  ...
  the gated policy P5 (54\%, retrieving 50\% of the time) beats always-retrieve
  P1/P4 (46\%, 100\%). Closed-book P3 (62\%) remains highest, reflecting that
  MIRAGE's multiple-choice questions largely test knowledge the model already
  holds.}
  \label{fig:medcorp-policies}
\end{figure}

\paragraph{3. On multiple choice, closed-book still leads --- a model-capability
ceiling.} P3 closed-book (62\%) remains above all retrieving policies. MIRAGE's
...
specific study findings, retrieval genuinely adds information, and P5 (F1 0.211)
edges above closed-book (0.190). Retrieval helps precisely when the model does not
...
low-risk bar: the best result on this benchmark, closed-book P3, is 62\%. This is
