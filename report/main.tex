...
%
%  BUILD FROM THE REPO ROOT, not from inside report/ — every generated table
%  and figure is referenced repo-root relative (results/tables, results/figures):
%      latexmk -pdf -outdir=report/build report/main.tex
%  or:  pdflatex report/main -> bibtex -> pdflatex report/main (x2)
%  On Overleaf, upload the repo root as the project and set report/main.tex as
%  the main document; Overleaf resolves \input against the project root.
%
%  Target length ~15,000 words (body only); currently ~14,700.
%  Chapters live in chapters/ and are pulled in with \input{}. The weekN sprint
%  files and the two pre-merge scaling chapters are retired — see the \input
...
% All generated assets are referenced repo-root relative: tables as
% \input{results/tables/...} and figures as "results/figures/fig_x". This
% document must therefore be compiled with the REPO ROOT as the working
% directory (see the build note above), not from inside report/.
...
drift-guarded, 240-test reproducibility harness.
\end{abstract}
