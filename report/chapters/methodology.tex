...
through shared, typed dataclasses} (Figure~\ref{fig:architecture}). The
practical consequence is that every
...
\begin{figure}[t]
  ...
  \caption{The six-layer architecture. Each layer depends only on those
    ...
    Chapter~\ref{ch:eval} --- threshold calibration, gate ablations and the
    paired distraction study --- replay from those logs without re-invoking
...
\begin{itemize}
  ...
  \item \textbf{P2 (always-retrieve with citations)} additionally asks the
    model to name its sources; following the attribution-evaluation
    convention of \citet{alce2023}, cited titles are matched leniently back
    to retrieved chunks and invented citations are dropped rather than
    counted.
...
\emph{Token entropy} is inflated by benign lexical freedom: probability
mass spreads wherever multiple phrasings are equally acceptable ---
formatting tokens, function words, punctuation --- regardless of whether
the underlying \emph{fact} is certain. A model that knows the answer but
has many ways to phrase it looks uncertain to an entropy gate. This is not
hypothetical: the token-level attribution analysis of
Section~\ref{sec:eval-attribution} demonstrates exactly this failure on
real clinical text, with entropy concentrating on filler tokens while the
clinically decisive tokens are generated at near-certainty.

\emph{Logit margin} fails in the opposite direction: its failure mode is
confident error. A model that has internalised a wrong fact produces a
large margin on a wrong answer, and the gate reads certainty. Margin
measures decisiveness, not correctness.

\emph{Verbalized confidence} --- asking the model to rate its own answer
--- is the standard third option, but \citet{kadavath2022} show
self-assessment calibration degrades with model size, and self-reports from
a 3B model are close to uninformative. This project therefore
\emph{replaces} verbalized confidence with a stability test in the spirit
of self-consistency \citep{wang2023selfconsistency}: generate two drafts
under different prompt framings and measure their agreement. Genuinely held
knowledge is invariant to surface framing; a guess is not. Reduced from
many-sample majority voting to a two-draft agreement test, the idea becomes
cheap enough to run per query on CPU.

The design response to these three failure modes is an ensemble of three
signals derived from \emph{different} aspects of the model's behaviour ---
distributional spread, decision-boundary decisiveness, and cross-framing
stability --- combined by majority vote so that no single failure mode
dominates.
