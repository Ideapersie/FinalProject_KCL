...
decoding and fixed seed, so every run is deterministic and was verified to
...
The sample size is conceded at the outset, since $n=\nQuestions{}$ per
condition is small and its cost is visible in the confidence intervals below.
A result that is a count over the sample, such as retrieval rate, is exact for
that sample, whereas an estimate of a population difference carries an
interval that is reported and taken seriously.

\section{Main Comparison}
\label{sec:eval-main}

\input{results/tables/tab_main_results}
...
($F_1 = \fOnePfiveOpen{}$) edges above closed-book P3 (\fOnePthreeOpen{}), with
P1 at \fOnePoneOpen{}. Where the model \emph{lacks} the specific study
...
any threshold without invoking the model, so a sweep over a threshold grid
...
The gate was functioning exactly as designed and firing at its calibrated
rate, yet firing made the policy worse because the corpus contained almost
nothing worth retrieving. The general principle is that \textbf{gated
retrieval inherits the quality of the corpus it retrieves from}, so on a poor
corpus the safest gate is one that retrieves less. Replacing the pilot with
the real corpus lifted P1 from 17\% to \accPoneMcq{}
(Figure~\ref{fig:corpus-effect}), which retrospectively confirms that the
earlier collapse was a property of the corpus rather than of the gating
machinery.
...
starts summarising, or else declines to answer a question it had, minutes
...
the gold answer in a chunk; on broken questions, only 13\%, so the chunks
...
real ceiling, since only 31\% gold-in-chunk even on the successes says the
...
interval on 19/19 spans [83.2\%, 100\%], so the honest statement is a
...
so it is protective where it skips, but it does not yet skip everywhere it
...
decisions, unsurprisingly, as both derive from the same draft's logits; they
...
measuring how often each member changes the ensemble outcome, attributes
24\% of decisions to entropy, 26\% to margin and 6\% to the probe. The ensemble is
...
retrieval, and the flat majority vote, by requiring a logit signal to concur
...
the method: the high-entropy tokens are \emph{filler and phrasing}, such as
newlines, punctuation, ``to'' and ``is'', while the model is near-certain on
...
is useful precisely because nobody should deploy without knowing it. That
attribution is tested directly by the scaling study
(Chapter~\ref{ch:scaling}): on the identical benchmark the 7B model clears the
low-risk bar, with closed-book P3 at \accQwenPthreeMcq{} and gated P5 at
\accQwenPfiveMcq{}, both above \barLow{}, so the envelope is empty here
because of scale, not method, and ceases to be empty at low risk once capability
is sufficient.
...
deployment where retrieval is the expensive resource, such as a large corpus
on slow storage, a metered search service or a shared index under load, halving
...
Every analysis in this chapter used the 3B reference model, and several of
its findings are claims about the method rather than about that model.
Chapter~\ref{ch:scaling} tests which of them survive by replicating the
pipeline unchanged at 7B and 14B.