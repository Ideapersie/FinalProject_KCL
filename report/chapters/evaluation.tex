...
\section{From One Model to Three}
\label{sec:eval-crossmodel}

Every analysis in this chapter used the 3B reference model, and several of
its findings are claims about the \emph{method} rather than about that
model: that thresholds must be calibrated per model, that retrieval
distracts, that selective retrieval beats always-retrieve. A method claim
supported by one instance is an anecdote. Which of these findings survive,
and which turn out to be artefacts of a small model, is the question
Chapter~\ref{ch:scaling} answers, by replicating the pipeline unchanged
(same corpus, same questions, same policy code, same quantisation) at 7B
and 14B.
