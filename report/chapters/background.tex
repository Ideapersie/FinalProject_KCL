...
The standard pipeline of retrieve, augment and generate embeds an assumption
...
fine-tuning of the generating model on curated reflection data, which is
infeasible
...
namely token-level entropy, top-two margin and related statistics, thresholded to
...
of retrieved snippets, so retrieval quantity matters. Second, a
``lost-in-the-middle'' effect penalises evidence buried mid-context, so
...
from. \textbf{Token-distribution statistics}, meaning the entropy of the
...
samples tracks correctness. Calibration in the formal sense, meaning
alignment between confidence and empirical accuracy, is measured by expected
...
therefore an open empirical question, and this dissertation answers it in
...
of the threshold-transfer failure, since the sharply peaked output distributions
...
known to the author combines all four. That combination, together with the
findings it surfaces, several of them negative and none of them visible in
aggregate accuracy alone, is the space this dissertation occupies.
